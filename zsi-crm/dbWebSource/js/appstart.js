...
var app = (function() {
    ...
    _app.init   = function(){
        ...
        app.getUserInfo(function(){
            if(app.userInfo.has_img === 'Y') $(".profile-image").attr({src: base_url + "dbimagebyclient/E1306/" + app.userInfo.client_id + "/" + userId }); 
            else $(".image-alt").text(app.userInfo.first_name.substring(0,1)).css("z-index", 3).show();
            if(app.userInfo.client_id === 0) $(".zsi-page-logo").attr({src: base_url + 'img/logo.png'});
            else $(".zsi-page-logo").attr({src: base_url + 'dbimage/ref-00012/client_id/' + app.userInfo.client_id + "/company_logo" }); 
        });
    ...
    _app.loadMenu                   = function(callback){
        ...
        if (isZSILogin!==null && isZSILogin!=="Y"){
