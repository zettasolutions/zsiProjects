...
BEGIN  
  ...
  IF ISNULL(@app_user_id,0) = 0
      SELECT NULL user_role_id, NULL as app_user_id, role_id, role_name FROM roles
  ELSE
  BEGIN
	  ...
	  SET @stmt = @stmt + ' UNION SELECT NULL user_role_id, NULL as app_user_id, role_id, role_name FROM roles where role_id not IN (select role_id from user_roles_v '
