...
function submitItems(){    
  $("#frm_modalWindow").jsonSubmit({
            ...
            ,optionalItems: ["store_id","supply_id"]
...
$("#btnSave").click(function () {
    $("#grid").jsonSubmit({
              procedure  : "supplies_upd"
               ,optionalItems : ["unit_id","supply_type_id","store_id","supply_id"]
             ,onComplete : function (data) {
                  $("#grid").clearGrid();
                  if(data.isSuccess===true) zsi.form.showAlert("alert");
                  displayRecords();
...
function displaySupplyBrands(id,sId){   
    sId = store_id;
     var cb = bs({name:"cbFilter1",type:"checkbox"});
        $("#" + tblName).dataBind({
        		...
        		{text  : cb                                   , width  : 25       , style    : "text-align:left;"  
                		   , onRender      :  function(d){ 
                		              return bs({name:"supply_brand_id"   ,value: svn (d,"supply_brand_id")    ,type:"hidden"})
                		                 +  bs({name:"supply_id"        ,value: id   ,type:"hidden"})
                                         +  bs({name:"store_id"   ,value: sId    ,type:"hidden"})
                                         +  (d !==null ? bs({name:"cb",type:"checkbox"}) : "" );
                            }
        		}
...
function manageItems(supply_id){
