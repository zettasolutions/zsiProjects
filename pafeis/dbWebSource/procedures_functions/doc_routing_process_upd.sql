...
BEGIN
  ...
  UPDATE dbo.doc_routings SET page_process_action_id=@page_process_action_id, acted_by = @user_id, acted_date=GETDATE()
   ...
   SELECT TOP 1 @page_process_id=next_process_id FROM dbo.page_process_actions WHERE page_process_action_id =@page_process_action_id
   ...
   BEGIN
	  ...
	  UPDATE dbo.doc_routings SET is_current = 'N' WHERE page_id=@page_id and doc_id=@doc_id AND ISNULL(page_process_action_id,0) <> 0 ;
   END
