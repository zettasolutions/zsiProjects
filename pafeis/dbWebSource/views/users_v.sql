...
SELECT        dbo.users.user_id, dbo.users.logon, dbo.users.last_name, dbo.users.first_name, dbo.users.middle_name, dbo.users.password, dbo.users.role_id, dbo.users.is_active, dbo.users.is_admin, 
                         ...
                         dbo.users.is_employee, dbo.users.is_developer, dbo.users.warehouse_id, dbo.getWarehouseLocation(dbo.users.warehouse_id) AS warehouse_location, 
                         dbo.users.last_name + N',' + dbo.users.first_name AS last_first_name, dbo.organizations_v.organization_type_code, dbo.squadron_types.squadron_type, dbo.squadron_types.squadron_type_id
FROM            dbo.users INNER JOIN
                         dbo.organizations_v ON dbo.users.organization_id = dbo.organizations_v.organization_id LEFT OUTER JOIN
                         dbo.squadron_types ON dbo.organizations_v.squadron_type_id = dbo.squadron_types.squadron_type_id
