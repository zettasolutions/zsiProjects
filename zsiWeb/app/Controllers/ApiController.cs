    ...
    public class ApiController : APIBaseController
    {
        ...
        public ContentResult Update()
        {
            try
            {
                using (new impersonate())
                {
                    ...
                    return Content(DataHelper.ProcessRequest(HttpContext.Request, ExecutionType.NonQuery,JsonRowsFormat.KeyValue,true), "application/json");
                }
            }
            catch (Exception ex)
            {
        ...
        public ContentResult GetRecords()
        {
            try
            {
                using (new impersonate())
                {
                    return Content(DataHelper.ProcessRequest(HttpContext.Request, ExecutionType.Reader, JsonRowsFormat.KeyValue, true), "application/json");
                }
            }
            catch (Exception ex)
            {
        ...
        public ContentResult GetRecords2()
        {
            try
            {
                using (new impersonate())
                {
                    return Content(DataHelper.ProcessRequest(HttpContext.Request, ExecutionType.Reader, JsonRowsFormat.Array,true), "application/json");
                }
            }
            catch (Exception ex)
            {
