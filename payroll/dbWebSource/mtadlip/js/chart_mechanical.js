...
function setChartSettings(){
    ...
    if(isContain(gMenu, "WIRES & CABLES")){
        _param += ",@table_view_name='dbo.wires_v'";
    }
        ...
        _param += ",@table_view_name='dbo.coverings_v'";
    }
    else if(isContain(gMenu, "RETAINERS")){
        gIsStacked = true;
        _param += ",@table_view_name='dbo.retainers_v'";
    }
    else if(isContain(gMenu, "GROMMETS")){
        _param += ",@table_view_name='dbo.grommets_v'";
    }
    else if(isContain(gMenu, "TROUGH/SHIELD/BRACKET")){
        _param += ",@table_view_name='dbo.stc_v'";
    }
...
function getDistinctKey(data){
    ...
    var _ground_circuit = "";
    var _classification = "";
    var _engagement_method = "";
    var _terminal_supplier = "";
    var _ring_eyelet_sealing = "";
    var _ring_eyelet_type = "";
    var _strand_config = "";
    var _covering_type = "";
    var _clip_type = "";
    var _supplier = "";
    ...
    if(data.length > 0){
        $.each(Object.keys(data[0]), function(i, key){
            ...
            else if(isContain(_key, "NO_OF_CAVITIES") || isContain(_key, "CAVITIES")){
                _no_cavities = key;
            }
            else if(isContain(_key, "COLOR")){
                _color = key;
            }
            else if(isContain(_key, "GROUND_CIRCUIT")){
                _ground_circuit = key;
            }
            else if(isContain(_key, "CLASSIFICATION")){
                _classification = key;
            }
            else if(isContain(_key, "ENGAGEMENT_METHOD")){
                _engagement_method = key;
            }
            else if(isContain(_key, "TERMINAL_SUPPLIER")){
                _terminal_supplier = key;
            }
            else if(isContain(_key, "RING_EYELET_SEALING")){
                _ring_eyelet_sealing = key;
            }
            else if(isContain(_key, "RING_EYELET_TYPE")){
                _ring_eyelet_type = key;
            }
            else if(isContain(_key, "STRAND_CONFIGURATION")){
                _strand_config = key;
            }    
            else if(isContain(_key, "COVERING_TYPE")){
                _covering_type = key;
            }
            else if(isContain(_key, "CLIP_TYPE")){
                _clip_type = key;
            }
            else if(isContain(_key, "SUPPLIER")){
                _supplier = key;
            }
            else{
                _category = key;
    ...
    _loc = (_location) ? _location : _alias_name;
    _cat = _category;
    
    // if(isContain(gMenu, "WIRES & CABLES")){
    //     if(isContain(gCName, "1 or 2 ways connector having 0.35 CSA and below")){
    //         _loc = _category;
    //         _cat = _no_cavities;
    //     }
    //     else if(isContain(gCName, "Directly terminated drain wires")){
    //         _loc = _alias_name;
    //         _cat = _device_name;
    //     }
    //     else if(isContain(gCName, "Wire color combination for ground circuits")){
    //         _loc = _color;
    //         _cat = _ground_circuit;
    //     }
    //     else if(isContain(gCName, "Flexible strand configuration for battery cable")){
    //         _loc = _strand_config;
    //     } 
    //     else if(isContain(gCName, "Shielded Wires not common with any noisy circuits")){

    //     }
    //     else if(isContain(gCName, "The insulation of airbag wires shall have orange color")){
    //         _loc = _color;
    //         _cat = _device_name;
    //     }
    ...
    //     }
    // }
    // if(isContain(gMenu, "INLINE CONNECTOR")){
    //     if(isContain(gCName, "Connector cavity ways with lever assist")){
    //         _loc = "";
    //         _cat = _no_cavities;
    //     }
    //     else if(isContain(gCName, "Non-waterproof connector located in the exterior compartment")){
    //         _loc = _category;
    //         _cat = _location;
    //     }
    //     else if(isContain(gCName, "Greased connectors should be avoided wherever possible")){
    //         _cat = _classification;
    //     }
    //     else if(isContain(gCName, "Strain relief of Inline connectors having SRS and Airbag")){
    //         _loc = _alias_name;
    //     }
    //     else if(isContain(gCName, "Lear Terminals")){
    //         _loc = _alias_name;
    //     } 
    //     else if(isContain(gCName, "Lear Connector")){
    //          _loc = _alias_name;
    //         _cat = _no_cavities;
    //     }
    //     else if(isContain(gCName, "2.5 wire gauge for cigar lighters and power outlet")){
    ...
    //     }
    // }
    // if(isContain(gMenu, "GROUND EYELET")){
    //     if(isContain(gCName, "Commonize Eyelet")){
    //         _loc = _ring_eyelet_type;
    //         _cat = _location;
    //     }
    //     else if(isContain(gCName, "New design or new grounding strategy used. Similar to ground block instead of eyelet")){
    //         _loc = _ring_eyelet_type;
    //         _cat = _ring_eyelet_sealing;
    //     }
    //     else if(isContain(gCName, "Eyelets not dip soldered or paint scraper/serrated type but having a good contact material")){
    //         _loc = _alias_name;
    //         _cat = _ring_eyelet_sealing;
    //     }
    //     else if(isContain(gCName, "No ground eyelets are allowed on seat structure or closure panels")){
           
    //     }
    //     else if(isContain(gCName, "Avoid Stacking Ring Terminals")){
    //         _loc = _ring_eyelet_type;
    //         _cat = _location;
    //     } 
    //     else if(isContain(gCName, "Battery negative eyelet type")){
    //         _loc = _ring_eyelet_type;
    //         _cat = _category;
    //     }
    // }
    // if(isContain(gMenu, "COVERING")){
    //     _loc = _covering_type;
    //     _cat = _location;
    //     if(isContain(gCName, "Abrasion resistant materials and location in the vehicle")){
            
    //     }
    //     else if(isContain(gCName, "Heat resistant materials and location in the vehicle")){

    //     }
    //     else if(isContain(gCName, "Conduit in the underbody environments to protect against stone impinge, grit and general exposure")){

    //     }
    //     else if(isContain(gCName, "Mostly method taping used")){
    //         _loc = _category;
    //     }
    //     else if(isContain(gCName, "Coverings Sleeve type")){
    //         _loc = _category;
    //     }
    // }
    // if(isContain(gMenu, "RETAINER")){
    //     _loc = _category;
    //     _cat = _location;
    //     if(isContain(gCName, "Commonize Retainer")){
    //         _loc = _clip_type;
    //     }
    //     else if(isContain(gCName, "Retainer with foam washer to prevent water ingress")){

    //     }
    //     else if(isContain(gCName, "Retainer Material such as PA66 having very good stiffness, Heat resistance")){

    //     }
    //     else if(isContain(gCName, "Retainer supplier")){
    //         _loc = _supplier;
    //     }
    // }
    // if(isContain(gMenu, "GROMMET")){
    //     _loc = _category;
    //     _cat = _location;
        
    //     if(isContain(gCName, "Grommet supplier")){
    //         _loc = _supplier;
    //         _cat = _category;
    //     }
    // }
    // if(isContain(gMenu, "SPLICE")){
    //     _loc = _category;
    //     _cat = _location;
        
    //     if(isContain(gCName, "Location of splice")){
    //         _loc = _location;
    //         _cat = _device_name;
    //     }
    // }
    // if(isContain(gMenu, "TROUGH/SHIELD/BRACKET")){
    //     _loc = _category;
    //     _cat = _location;
        
    //     if(isContain(gCName, "Channel location")){
    //         _loc = _location;
    //         _cat = _category;
    //     }
    //     else if(isContain(gCName, "Usage of Spline")){
    //         _loc = "";
    //         _cat = _location;
    //     }
    // }
    // if(isContain(gMenu, "BATTERY FUSE TERMINAL")){
    //     _loc = _ring_eyelet_sealing;
    //     _cat = _ring_eyelet_type;
    // }
...
function displayDetailsRegionBar(container, data){
    var _key = getDistinctKey(gData);
    var _value = _key.value;
    var _category = _key.category;
    var _region = _key.region;
    ...
    // var _oemFirst = _index.first;
    // var _oemLast = _index.last;
    // var _harnessList = getFirstAndLastItem(gHarness , "name");
    
    //$.each(gOEMs, function(i, v) { 
        var _index = getFirstAndLastItem(gOEMs , "name");
        ...
        $.each(_selectedCategory, function(i, v) { 
            ...
            _createLabel(_first, _last, _sName, 10);
        });
    //});
