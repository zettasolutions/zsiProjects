...
$("select[name='organization_filter']").dataBind({
    url: app.procURL + "organizations_dd_sel" 
...
function displayRecords(col_name,keyword){
    ...
    var rownum=0;
    $("#grid").dataBind({
         url            : app.procURL + "employees_sel @col_name='"+col_name
        ...
        ,dataRows : [
                { text  : cb, width : 25, style : "text-align:left;"
                    , onRender      :  function(d) {
                        return     app.bs({name:"user_id",type:"hidden",value: svn (d,"user_id")})
            ...
            $("select[name='organization_id']").each(function(){
                ...
                if(this.hasAttribute("selectedvalue")) {
                    ...
                    $zRow.find("select[name='warehouse_id']").dataBind({
                        url: app.execURL + "warehouses_sel @squadron_id=" + id
            ...
            $("select[name='organization_id']").dataBind({
                ...
                , onComplete : function(){
                    $("select[name='organization_id']").change(function(){
                        ...
                        if(this.value) {
                            $zRow.find("select[name='warehouse_id']").dataBind({
                                url: app.execURL + "warehouses_sel @squadron_id=" + this.value
                                , text: "warehouse_location"
                                , value: "warehouse_id"
                            });  
                        }else{
...
function displayInactive(){
    ...
    $("#inActiveRecords").dataBind({
         url            : app.procURL + "employees_sel @is_active='N'" 
    ...
    $.ajax({
        ...
        success: completeHandler = function(data) {
            if(data.isSuccess) {
                ...
                $.get(app.execURL  + "dbo.image_file_users_upd @user_id=" + user_id
