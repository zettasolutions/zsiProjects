...
IF ISNULL(@user_id,0) <> 0
  SELECT @is_dev = is_developer FROM dbo.users_v where user_id=@user_id;
ELSE
IF ISNULL(@logon,'')<>''
SELECT @is_dev = is_developer FROM dbo.users_v where logon=@logon;
*/ 
SET @stmt = 'SELECT user_id, userFullName, logon, first_name, middle_name, last_name, name_suffix, password,  is_admin, role_id, is_developer, company_id, img_filename, hash_key  FROM users_v WHERE is_afcs=''Y'' and is_active = ''' + CAST(@is_active AS VARCHAR(1)) + '''';  
