...
CREATE PROCEDURE [dbo].[ztime_register_upd]  
(  
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SELECT 
					'N' is_valid
					, 'The QR code is already registered to another device. Please contact support.' AS msg
			END
			ELSE
			BEGIN
				BEGIN TRAN;
				UPDATE zsi_crm.dbo.devices SET 
					  is_active = 'Y'
