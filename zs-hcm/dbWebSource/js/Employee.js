var employees = (function(){
                ...
                ,dataRows   : [
        		    ...
        		    { text:"Photo"             , width:40      , style:"text-align:center;" 
            		    ,onRender : function(d){ 
                            var image_url = base_url + "dbimagebyclient/e247/" + _clientId + "/" + app.svn(d,"id") + "?ts=" + new Date().getTime();                               
                            var mouseMoveEvent= "onmouseover='employees.mouseover(\"" + image_url  + "\");' onmouseout='employees.mouseout();'";  
                            var html = "<a href='javascript:void(0);' "+ mouseMoveEvent +" class='btn btn-sm has-tooltip' onclick='employees.showProfileImage(" + svn(d,"id") +");' data-toggle='tooltip' data-original-title='Upload Image'><i class='fas fa-image'></i> </a>";
                            return (d!==null ? html : "");
                        }
        		    }
                ...
                _zRow.find("[name='position_id']").dataBind({
                    ...
                    ,onChange     : function(d){
                        var  _info       = d.data[d.index - 1]
                            ...
                            _$zRow.find('[name="basic_pay"]').val(commaSeparateNumber(_basic_pay));
                    }
                }); 
    ...
    _public.showProfileImage = function(empId){
        _sqlCode = "U252";
        _gEmpId = empId;
        $("#myModalViewUploadImg").find("#image").cropper('destroy').attr({src: base_url + "dbimagebyclient/E247/" + app.userInfo.company_id + "/" + empId }); 
    ...
    _public.showModalUploadEmplImage = function(emplId, name){ 
        ...
        var m=$('#' + mdlImageEmpl);
        m.find(".modal-title").text('Upload Photo for ' + name);
        m.modal("show");
